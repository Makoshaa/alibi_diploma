...
// Получаем базовый URL для wasm файлов
const getWasmBaseUrl = () => {
  const apiBaseUrl = import.meta.env.VITE_API_BASE_URL;
  if (apiBaseUrl) {
    // Убираем /api из конца
...
export function loadLivenessModule() {
  ...
  loadingPromise = new Promise((resolve, reject) => {
    ...
    Module.locateFile = (path) => `${baseUrl}/wasm/${path}`;
    ...
    script.src = `${baseUrl}/wasm/Emscrippeng_test.js`;
    script.async = true;
